...
var process_cluster_template = function (template_file, conf) {
  ...
  // replace parameters with conf.resources
...
//  obj.Resources.MasterInstance.Properties.UserData = fs.readFileSync(util.join_output_file_path(conf.resources.cluster_name, 'master_node.yaml')).toString('base64');
//  obj.Resources.WorkerLaunchConfiguration.Properties.UserData = fs.readFileSync(util.join_output_file_path(conf.resources.cluster_name, 'worker_node.yaml')).toString('base64');

  obj.Resources.MasterInstance.Properties.UserData = zlib.gzipSync(fs.readFileSync(util.join_output_file_path(conf.resources.cluster_name, 'master_node.yaml'))).toString('base64');
  obj.Resources.WorkerLaunchConfiguration.Properties.UserData = zlib.gzipSync(fs.readFileSync(util.join_output_file_path(conf.resources.cluster_name, 'worker_node.yaml'))).toString('base64');
